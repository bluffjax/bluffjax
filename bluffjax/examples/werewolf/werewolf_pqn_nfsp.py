    ...
    def run_compare_eval(
        ...
        def compare_episode(carry, episode_idx):
            ...
            (
                ...
                avg_human_win,
                avg_werewolf_win,
            ) = compare_single_episode(rng_s, avg_p, base_p, train_idx, True, br_net, avg_net)
            (
                rng_s,
                br_ret,
                br_len,
        ...
        def update_step(
            ...
            def _reshape_transition(trans: Transition) -> Transition:
                ...
                return Transition(
                    ...
                    roles=_reshape_batch(trans.roles),
                    info=info_reshaped,
                )
            ...
            def update_epoch(
                ...
                trans_shuffled = Transition(
                    ...
                    roles=jnp.take(trans_reshaped.roles, permutation, axis=0),
                    info=info_shuffled,
                )
                ...
                minibatches_trans = Transition(
                    ...
                    roles=trans_shuffled.roles.reshape(
                        config["num_minibatches"], minibatch_size, -1
                    ),
            ...
            human_team_win_rate_rollout = (
                (game_winner_signal * done_mask * human_mask).sum()
                / jnp.maximum((done_mask * human_mask).sum(), 1.0)
            )
            werewolf_team_win_rate_rollout = (
                (game_winner_signal * done_mask * werewolf_mask).sum()
                / jnp.maximum((done_mask * werewolf_mask).sum(), 1.0)
            )
            ep_length_avg = (
                jnp.where(transitions.done, transitions.info["timestep"], 0.0).sum()
...
def main(config: dict) -> None:
    ...
    if config["save_final"]:
        ...
        br_save_path = os.path.join(
            config["save_dir"], f"werewolf_pqn_nfsp_br_{timestamp}.msgpack"
        )
        avg_save_path = os.path.join(
            config["save_dir"], f"werewolf_pqn_nfsp_avg_{timestamp}.msgpack"
        )
