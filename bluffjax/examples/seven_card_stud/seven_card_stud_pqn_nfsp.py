"""
PQN + NFSP for Seven-Card-Stud (AEC).
"""
...
def make_train(config: dict) -> Callable[[PRNGKeyArray, int], RunnerState]:
    ...
    def compare_single_episode(
        ...
        def play_body(carry):
            ...
            train_return_now = reward[train_player_idx]
            train_win_now = (train_return_now > 0.0).astype(jnp.float32)
...
def main(config: dict) -> None:
    ...
    if config["save_final"]:
        ...
        br_save_path = os.path.join(
            config["save_dir"], f"seven_card_stud_pqn_nfsp_br_{timestamp}.msgpack"
        )
        avg_save_path = os.path.join(
            config["save_dir"], f"seven_card_stud_pqn_nfsp_avg_{timestamp}.msgpack"
        )
