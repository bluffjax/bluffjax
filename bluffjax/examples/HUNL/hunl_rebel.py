"""
ReBeL (Recursive Belief-based Learning) for heads-up no-limit Texas Hold'em.
...
from bluffjax.examples.HUNL.hunl_game_utils import (
    NUM_ACTIONS,
    NUM_BUCKETS,
    PBS_INPUT_DIM,
    VALUE_OUTPUT_DIM,
...
def _apply_path(
    env: TexasNoLimitHoldem,
    root_state: TexasNoLimitHoldEmState,
...
class RunnerState(NamedTuple):
    ...
    env_state: TexasNoLimitHoldEmState
    rng: PRNGKeyArray
...
def make_train(config: dict) -> Callable[[PRNGKeyArray, IntArray], dict]:
    ...
    def train(rng: PRNGKeyArray, exp_id: IntArray) -> dict:
        ...
        env = make("texas_nolimit_holdem", **config.get("env_kwargs", {"num_agents": 2}))
    ...
    try:
        ...
        if config["save_final"] and "params" in result:
            ...
            save_path = os.path.join(
                config["save_dir"], f"hunl_rebel_{timestamp}.msgpack"
            )
