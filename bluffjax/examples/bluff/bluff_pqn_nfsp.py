...
from bluffjax.environments.bluff.bluff import BluffState
...
class Transition(NamedTuple):
    ...
    action: IntArray
    reward: FloatArray  # acting player reward
    reward_full: FloatArray  # full reward vector for metrics
    absorbing: BoolArray
    done: BoolArray
    q_val: FloatArray
    player_idx: IntArray
    next_obs: FloatArray
    is_br: BoolArray
    play_action_is_lie: BoolArray
    play_action_valid: BoolArray
    info: dict[str, Any]
...
def make_train(config: dict) -> Callable[[PRNGKeyArray, int], RunnerState]:
    ...
    if compare_enabled and (not compare_against_random):
        ...
        if compare_network_type == "actor_critic":
            ...
            template_params = actor_critic_network.init(
                jax.random.PRNGKey(0), sample_obs
            )
            ...
            template_params = baseline_q_network.init(jax.random.PRNGKey(0), sample_obs)
            with open(checkpoint_path, "rb") as f:
                baseline_params = serialization.from_bytes(template_params, f.read())
            baseline_params = jax.tree_util.tree_map(
                _squeeze_leading_batch, baseline_params
    ...
    def compare_single_episode(
        ...
        br_net: QNetworkDiscreteMLP,
        avg_net: ActorDiscreteMLP,
    ) -> tuple[PRNGKeyArray, FloatArray, FloatArray, FloatArray, FloatArray]:
        ...
        _, _, rng, step_count, done_flag, terminal_rewards, terminal_train_win = (
            lax.while_loop(
                play_cond,
                play_body,
                (
    ...
    def run_compare_eval(
        ...
        def compare_episode(carry, episode_idx):
            ...
            rng_s, avg_ret, avg_len, avg_done, avg_win = compare_single_episode(
                rng_s, avg_p, base_p, train_idx, True, br_net, avg_net
            )
            rng_s, br_ret, br_len, br_done, br_win = compare_single_episode(
                rng_s, br_p, base_p, train_idx, False, br_net, avg_net
            )
        ...
        return rng, avg_ret, br_ret, avg_eval_len, avg_win_rate, br_win_rate
        ...
        def update_step(
            ...
            def step(
                ...
                random_action = jax.vmap(
                    lambda rng_i, p: distrax.Categorical(probs=p).sample(seed=rng_i)
                ...
                play_action_valid = (
                    (state_s.phase == 1)
                    & state_s.has_current_rank
                    & (~state_s.rank_choice_pending)
                )
                play_action_is_lie = play_action_valid & (action != 0)
                ...
                transition = Transition(
                    ...
                    next_obs=next_obs,
                    is_br=is_br,
                    play_action_is_lie=play_action_is_lie,
                    play_action_valid=play_action_valid,
                    info=info,
                )
                runner_state_inner = RunnerState(
            ...
            def compute_q_lambda_targets(
                ...
                reward_s = transitions_s.reward
                next_obs_s = transitions_s.next_obs.astype(jnp.float32)
                done_s = transitions_s.done
                q_vals_next = br_network.apply(br_params, next_obs_s)

                done_f32 = done_s.astype(jnp.float32)
                lambda_returns_last = (
                    reward_s[-1] + gamma * (1.0 - done_f32[-1]) * last_q_s
                )
            ...
            def _reshape_transition(trans: Transition) -> Transition:
                ...
                return Transition(
                    ...
                    play_action_is_lie=_reshape_batch(trans.play_action_is_lie),
                    play_action_valid=_reshape_batch(trans.play_action_valid),
                    info=info_reshaped,
                )
            ...
            def update_epoch(
                ...
                trans_shuffled = Transition(
                    ...
                    play_action_is_lie=jnp.take(
                        trans_reshaped.play_action_is_lie, permutation, axis=0
                    ),
                    play_action_valid=jnp.take(
                        trans_reshaped.play_action_valid, permutation, axis=0
                    ),
                    info=info_shuffled,
                )
                ...
                minibatches_trans = Transition(
                    ...
                    play_action_is_lie=trans_shuffled.play_action_is_lie.reshape(
                        config["num_minibatches"], minibatch_size
                    ),
                    play_action_valid=trans_shuffled.play_action_valid.reshape(
                        config["num_minibatches"], minibatch_size
                    ),
            ...
            play_action_count = jnp.maximum(
                transitions.play_action_valid.astype(jnp.float32).sum(), 1.0
            )
            play_action_lie_frac = (
                transitions.play_action_is_lie.astype(jnp.float32).sum()
                / play_action_count
            )
            ...
            metric = {
                ...
                "play_action_lie_frac": play_action_lie_frac,
    ...
    try:
        ...
        if config["save_final"]:
            ...
            br_save_path = os.path.join(
                config["save_dir"], f"bluff_pqn_nfsp_br_{timestamp}.msgpack"
            )
            avg_save_path = os.path.join(
                config["save_dir"], f"bluff_pqn_nfsp_avg_{timestamp}.msgpack"
            )
