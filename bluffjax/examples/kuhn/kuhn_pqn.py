...
def make_train(config: dict) -> Callable[[PRNGKeyArray, int], RunnerState]:
    ...
    def train(rng: PRNGKeyArray, seed: int) -> RunnerState:
        def train_setup(
            rng: PRNGKeyArray,
        ) -> tuple[TrainState, QNetworkDiscreteMLP, KuhnState, FloatArray]:
        ...
        def update_step(
            ...
            def step(
                ...
                q_vals = network.apply(train_state.params, obs)
            ...
            # Zero-sum: next_obs is from opponent's perspective, so our value = -max Q
            last_q_raw = jnp.max(network.apply(train_state.params, last_obs), axis=-1)
            last_q = -last_q_raw
            ...
            def compute_q_lambda_targets(
                ...
                q_vals = network.apply(train_state.params, transitions.obs)
                q_vals_next = network.apply(train_state.params, next_obs)

                lambda_returns_last = (
                    reward[-1]
                    + config["gamma"] * (1.0 - done[-1].astype(jnp.float32)) * last_q
                )
                # For the first scan iteration (step T-2), we need next_q = -max Q(obs[T-1])
                ...
                def _get_target(carry, rew_q_done):
                    ...
                    # lambda_returns from prev iter is opponent's target; our target uses -opponent
            ...
            def update_epoch(update_state: Updatestate, unused: None) -> Updatestate:
                ...
                def update_minibatch(
                    ...
                    def loss_fn(params, trans, targets_mb):
                        q_vals = network.apply(params, trans.obs)
                        chosen_q = jnp.take_along_axis(
                            q_vals,
                    ...
                    (td_loss, aux), grads = jax.value_and_grad(loss_fn, has_aux=True)(
            ...
            def logging_callback(seed_val, metric_dict, info, params):
                policy_net = policy_from_qnetwork(network.apply, params)
