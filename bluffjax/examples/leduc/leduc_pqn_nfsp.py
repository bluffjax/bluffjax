...
def make_train(config: dict) -> Callable[[PRNGKeyArray, int], RunnerState]:
    ...
    def train(rng: PRNGKeyArray, seed: int) -> RunnerState:
        def train_setup(
            ...
            avg_network = ActorCriticDiscreteMLP(
                action_dim=env.num_actions, hidden_dim=config["fc_dim_size"]
            )
            ...
            sl_buffer = init_sl_buffer(obs.shape[-1], env.num_actions)
            return (
                br_train_state,
                avg_train_state,
                br_network,
        ...
        def update_step(
            ...
            def step(
                ...
                avg_logits, _ = avg_network.apply(
            ...
            def logging_callback(seed_val, metric_dict, info, avg_params, br_params):
                avg_policy_arr = policy_array_from_network(
                    avg_network.apply, avg_params, _INFOSET_LIST
                )
                br_policy_arr = policy_array_from_qnetwork(
                    br_network.apply, br_params, _INFOSET_LIST
                )
                avg_expl = float(_exploitability_jitted(avg_policy_arr))
                br_expl = float(_exploitability_jitted(br_policy_arr))
                metric_dict = dict(metric_dict)
                metric_dict["exploitability_avg_policy"] = avg_expl
                metric_dict["exploitability_br_policy"] = br_expl
